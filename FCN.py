...
# 3) Download pretrained vgg16 model and put in model_path (should be done automatically if you have internet connection)
...
Train_Image_Dir = "Data_Zoo/Materials_In_Vessels/Train_Images/"
...
Train_Label_Dir = "Data_Zoo/Materials_In_Vessels/LiquidSolidLabels/"
# do you want to use validation set in training
UseValidationSet = False
# Validation images that will be used to evaluate training
Valid_Image_Dir = "Data_Zoo/Materials_In_Vessels/Test_Images_All/"
#  (the  Labels are in same folder as the training set)
Valid_Labels_Dir = "Data_Zoo/Materials_In_Vessels/LiquidSolidLabels/"
...
# Where train losses will be writen
TrainLossTxtFile = logs_dir + "TrainLoss.txt"
# Where validation losses will be writen
ValidLossTxtFile = logs_dir + "ValidationLoss.txt"
# Number of files per training iteration
Batch_Size = 2
# Weight for the weight decay loss function
Weight_Loss_Rate = 5e-4
# Max  number of training iteration
MAX_ITERATION = int(100010)
# Number of class for fine grain +number of class for solid liquid+Number of class for empty none empty +Number of class for vessel background
NUM_CLASSES = 4
...
def main(argv=None):
    ...
    # Create the net and load intial weights
    ...
    Loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.squeeze(GTLabel, squeeze_dims=[3]),
                                                                          logits=Net.Prob, name="Loss")))
    ...
    if UseValidationSet:
        ValidReader = Data_Reader.Data_Reader(Valid_Image_Dir, GTLabelDir=Valid_Labels_Dir,
    ...
    for itr in range(MAX_ITERATION):
        ...
        if UseValidationSet and itr % 2000 == 0:
            ...
            for i in range(NBatches):  # Go over all validation image
                Images, GTLabels = ValidReader.ReadNextBatchClean()  # load validation image and GT labels
